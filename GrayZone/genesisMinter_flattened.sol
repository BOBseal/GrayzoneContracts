...
contract ZONEPASS is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    address _feeToken;
    uint256 feeAmt = 1;
    uint256 directFee = 0.0001 ether; // fee per week
    bool internal d;
    ...
    struct PASSHOLDER {
        ...
        mapping(uint256 => string) _storageSlots;
    }
    mapping(address => uint256) internal contractAccruedFee;
    ...
    function addToStorage(string memory hash , uint256 id) public payable isMinter{
        ...
        _zonepassmaps[id]._storageSlots[a] = hash;
    }

    function delFromStorage(uint256 id, uint256 slot) public payable isMinter{
        require(_zonepassmaps[id].usedSlots <=_zonepassmaps[id].allowedSlots,"slot exceed");
        _zonepassmaps[id].usedSlots ++;
        incrementTotalServices(id);
        delete _zonepassmaps[id]._storageSlots[slot];
    }

    function getStorage( uint256 id , uint256 slot) public view returns(string memory){
        require(msg.sender == _zonepassmaps[id].holder,"not owner of id");
        return _zonepassmaps[id]._storageSlots[slot];
    }

    function getStorage_(address by, uint256 id , uint256 slot) public isMinter view returns(string memory){
        require(by == _zonepassmaps[id].holder,"not owner of id");
        return _zonepassmaps[id]._storageSlots[slot];
    }
    ...
    function mint(address to, string memory uri) public payable isMinter {
        ...
        _zonepassmaps[tokenId].allowedSlots = 300;
    }
    ...
    function withdraw(uint256 id , address token , uint256 amt , address to) public payable isMinter{
        require(id < _tokenIdCounter.current(),"non existent id");
    ...
    function balTransfer_(uint256 fromId , uint256 toId , address token , uint256 amt) public payable isMinter{
        ...
        _zonepassmaps[toId]._20bal[token] = _zonepassmaps[toId]._20bal[token] + amount;
        withdraw(fromId, token , fee_/2 , _zonepassmaps[fromId].minter);
        contractAccruedFee[token] = contractAccruedFee[token] + fee_/2;
...
contract GenesisMint {
    ...
    address owner;
    uint256 public dropPrice = 0.005 ether;
    ...
    function Claim() public{
        require(IERC20(dropFeeToken).balanceOf(msg.sender)>= dropPrice,"Fee not enough");
